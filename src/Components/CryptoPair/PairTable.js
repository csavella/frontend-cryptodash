...
export default function PairTable() {
  ...
  const market_data_url =
    "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=%271h%2C24h%27";
  ...
  useEffect(() => {
    let options = [];
    selectCoinsList.map((element) => options.push(element.name));
  ...
  useEffect(() => {
    let options2 = [];
    selectVsCurrencies.map((element) => options2.push(element.toUpperCase()));
          ...
          content={
            <div className="box-content">
              <div className="popup-header-div">
                <b id="popup-header">
                  Comparing {selectCoins[0]} vs. {selectCoins[1].toUpperCase()}
                </b>
                <button value="button" onClick={handleClose} id="x-btn">
                  x
                </button>
              </div>
              <ReactTable
              ...
              <div className="popup-btn-div">
                ...
                <button id="add-to-table-btn" onClick={handleAddToTable}>
                  Add to Table
                </button>
              </div>
            </div>
